...
public class KeyboardIOHandler {
	...
	public KeyboardIOHandler(KeyboardView keyboardView, MIDIInstrument instrument) {
		...
		for(int k : KEY_IDS) {
			Button key = (Button)(keyboardView.findViewById(k));
			OnTouchListener keyTouchListener = new OnTouchListener() {
				@Override
				public boolean onTouch(View touchedKey, MotionEvent event) {
					catchRogues();
			...
			ViewTreeObserver observer = key.getViewTreeObserver();
	...
	private void catchRogues() {
		Iterator<Integer> iterator = currentlyPressed.iterator();
